...
- 프론트엔드 앱: Next.js 기반 프론트엔드입니다.
- 백엔드 플랫폼: Kotlin/Spring Boot 기반 멀티모듈 백엔드입니다.

## 백엔드 플랫폼

백엔드 플랫폼은 Gradle Kotlin DSL을 사용하는 멀티모듈 프로젝트입니다.
...
공통 설정은 루트 `build.gradle.kts`에서 관리합니다.
...
```text
prompt-service/src/main/kotlin/.../
...
```text
ai-proxy-service/src/main/kotlin/.../
...
루트 `docker-compose.yml`은 로컬 개발용 인프라를 정의합니다.
...
프론트엔드 앱은 Next.js App Router 기반입니다.
